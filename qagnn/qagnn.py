...
def evaluate_accuracy(eval_set, model):
	...
	with torch.no_grad():
		for qids, labels, *input_data in eval_set:
			logits, _, concept_ids, gnn_output = model(*input_data)
			n_correct += (logits.argmax(1) == labels).sum().item()
			n_samples += labels.size(0)
			all_ids.append(concept_ids)
			all_outputs.append(gnn_output)

	all_ids = torch.cat(all_ids, dim=0)
	all_outputs = torch.cat(all_outputs, dim=0)

	torch.save(all_ids, "./ids.pt")
...
def main():
	...
	# data
	parser.add_argument('--num_relation', default=39, type=int, help='number of relations')
...
def train(args):
	...
	if True:
		...
		model = LM_QAGNN(args, args.encoder, k=args.k, n_ntype=5, n_etype=args.num_relation, n_concept=concept_num,
		...
		for epoch_id in range(args.n_epochs):
			...
			for qids, labels, *input_data in dataset.train():
				...
				for a in range(0, bs, args.mini_batch_size):
					...
					logits, _, concept_ids, gnn_output = model(*[x[a:b] for x in input_data], layer_id=args.encoder_layer)
...
def eval_detail(args):
	...
	save_test_preds = args.save_model
	if not save_test_preds:
		test_acc = evaluate_accuracy(dataset.dev(), model) if args.test_statements else 0.0
	else:
		eval_set = dataset.test()
		total_acc = []
		count = 0
