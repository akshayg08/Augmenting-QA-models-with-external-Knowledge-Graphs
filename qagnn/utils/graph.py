...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
	qc_ids, ac_ids, question, ec = data
	ec_nodes = set(ec)
	qa_nodes = set(qc_ids) | set(ac_ids)
	extra_nodes = set()
	for qid in qa_nodes:
	...
	extra_nodes = extra_nodes - ec_nodes
	
	return (sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes), sorted(ec_nodes))

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
	qc_ids, ac_ids, question, extra_nodes, ec = data
	cid2score = get_LM_score(qc_ids+ac_ids+extra_nodes+ec, question)
	return (qc_ids, ac_ids, question, extra_nodes, ec, cid2score)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
	qc_ids, ac_ids, question, extra_nodes, ec, cid2score = data
	# print(len(extra_nodes+qc_ids+ac_ids), len(cid2score), "HIHI")
	schema_graph = qc_ids + ac_ids + sorted(ec, key=lambda x: -cid2score[x]) + sorted(extra_nodes, key=lambda x: -cid2score[x]) #score: from high to low
	arange = np.arange(len(schema_graph))
	qmask = arange < len(qc_ids)
	amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
	ecmask = (arange >= len(qc_ids) + len(ac_ids)) & (arange < (len(qc_ids) + len(ac_ids) + len(ec)))
	adj, concepts = concepts2adj(schema_graph)
	return {'adj': adj, 'concepts': concepts, 'qmask': qmask, 'amask': amask, "ecmask": ecmask, 'cid2score': cid2score}
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, extra_concepts_path, num_processes):
	"""
	This function will save
		(1) adjacency matrics (each in the form of a (R*N, N) coo sparse matrix)
		(2) concepts ids
	...
	extra_concepts = pickle.load(open(extra_concepts_path, "rb"))
	for i in range(len(extra_concepts)):
		for j in range(len(extra_concepts[i])):
			extra_concepts[i][j] = concept2id[extra_concepts[i][j]]
	qa_data = []
	...
	with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
		...
		for j, line in enumerate(zip(lines_ground, extra_concepts)):
			...
			qa_data.append((q_ids, a_ids, QAcontext, ec))
