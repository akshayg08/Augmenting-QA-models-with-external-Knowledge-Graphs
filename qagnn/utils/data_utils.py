...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice, args):
	...
	if use_cache:
		...
		for idx, _data in tqdm(enumerate(adj_concept_pairs), total=n_samples, desc='loading adj matrices'):
			adj, concepts, qm, am, ecm, cid2score = _data['adj'], _data['concepts'], _data['qmask'], _data['amask'], _data['ecmask'],_data['cid2score']
			...
			node_type_ids[idx, 1:num_concept][torch.tensor(ecm, dtype=torch.bool)[:num_concept-1]] = 4
			...
			#Prepare edges
			i += 3; j += 1; k += 1  # **** increment coordinate by 1, rel_id by 3 ****
			...
			for _coord, ec_tf in enumerate(ecm):
				_new_coord = _coord + 1
				if _new_coord > num_concept:
					break
				if ec_tf:
					extra_i.append(2) #rel from contextnode to answer concept
					extra_j.append(0) #contextnode coordinate
					extra_k.append(_new_coord) #extra concept coordinate

			half_n_rel += 3 #should be 20 now
			if len(extra_i) > 0:
