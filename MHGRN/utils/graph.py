...
def concepts_to_adj_matrices_2hop_all_pair(data):
	...
	extra_nodes = list(extra_nodes - qa_nodes)
	num_nodes_to_delete = (len(extra_nodes) * remove_percent) // 100
	for i in range(num_nodes_to_delete):
		node = random.choice(extra_nodes)
		extra_nodes.remove(node)
	...
	if len(qc_ids) > 3:
		for i in range(3):
			node = random.choice(qc_ids)
			qc_keep.append(node)
			qc_ids.remove(node)

	if len(ac_ids) > 2:
		for i in range(2):
			node = random.choice(ac_ids)
			ac_keep.append(node)
			ac_ids.remove(node)

	schema_graph = sorted(qc_ids) + sorted(ac_ids) + sorted(extra_nodes)
	num_nodes_to_delete = (len(schema_graph) * remove_percent) // 100
	for i in range(num_nodes_to_delete):
		node = random.choice(schema_graph)
		schema_graph.remove(node)

	schema_graph = sorted(qc_keep) + sorted(ac_keep) + sorted(schema_graph)
...
def generate_adj_matrices(ori_schema_graph_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes, num_rels=34, debug=False):
	...
	global concept2id, id2concept, relati
	res = list(tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair, qa_data), total=len(qa_data)))

	# res is a list of tuples, each tupleon2id, id2relation
	if any(x is None for x in [concept2id, id2concept, relation2id, id2relation]):
		load_resources(cpnet_vocab_path)
...
def generate_adj_data_from_grounded_concepts(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes, K=0):
	"""
	...
	global concept2id, id2concept, relation2id, id2relation, cpnet_simple, cpnet, remove_percent
	remove_percent = K
	if any(x is None for x in [concept2id, id2concept, relation2id, id2relation]):
		load_resources(cpnet_vocab_path)
	if cpnet is None or cpnet_simple is None:
