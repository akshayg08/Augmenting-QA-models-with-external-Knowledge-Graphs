...
def check(question):
	negations = [" no ", " not ", " never ", " neither ", " nothing ", " barely ",
					" hardly ", " none ", " no one ", " nobody ", " nowhere ", " scarcely ", "n't "]
...
def decode(args):
	...
	print()
	print("***** decoding *****")
	print(dataset.dev_size(), dataset.test_size(), dataset.train_size())
	print(f'| dataset: {old_args.dataset} | num_dev: {dataset.dev_size()} | num_test: {dataset.test_size()} | save_dir: {args.save_dir} |')
	neg_cnt = 0
	tot_neg = 0
	non_neg_cnt = 0
		...
		with torch.no_grad():
			for qids, labels, *input_data in eval_set:
				...
				for i, (qid, label, pred) in enumerate(zip(qids, labels, predictions)):
					if filename == "decode_test.txt":
						print(statement_dic[qid]['question'].lower().strip())
						if check(statement_dic[qid]['question'].lower().strip()):
							if statement_dic[qid]['answers'][label.item()] == statement_dic[qid]['answers'][pred.item()]:
								neg_cnt += 1
							tot_neg += 1
	...
	print(tot_neg, tot_non_neg)
	print(neg_cnt / tot_neg)
	print(non_neg_cnt / tot_non_neg)
	print((neg_cnt + non_neg_cnt) / (tot_non_neg + tot_neg))
